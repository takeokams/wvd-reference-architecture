...
- Azure Firewall PremiumのTLS Inspectionで使う証明書に、Active Directory Certificate Servicesで作成したEnterprise Root CAで署名した、Intermediate CAをセットします。具体的には、既存であるKey Vaultに自分に対してアクセス権限を付与し、統合されていないCAによって発行された証明書を作成してCSRを出力し、Enterprise Root CAで署名してからKey Vaultに取り込んで、ファイアウォールポリシーのTLS Inspectionで証明書を付け替えます
